...
- The answer is generated by a local `knowledge-agent-v1` response helper, not a
  remote LLM call.
- Evidence still comes from static/demo knowledge plus uploaded in-memory
  ingestion data.
- The runtime tables make the chat observable and resumable, but they do not
  make document ingestion, chunks, or embeddings production-persistent yet.
