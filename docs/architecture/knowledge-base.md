...
Status: current API MVP plus target architecture notes. The current runtime has
relational persistence for source documents, chunks, ingestion/extraction jobs,
object links, and Agent runtime rows. Sections labeled **Target** describe the
next production retrieval layer, especially external embedding/vector
infrastructure.
...
The MVP now uses a hybrid model: persistent rows for document/chunk/runtime
state, with in-code/demo catalog concepts still used where the UI needs seeded
spaces, cards, and workflow examples.
...
| `knowledge_chunks` | Retrieval chunks with source metadata |
| `knowledge_ingestion_jobs` | Ingestion job status and diagnostics |
| `knowledge_extraction_jobs` | OCR/extraction job status and diagnostics |
...
```text
seed/demo catalog + persistent document rows
  -> upload simulation and ingestion/extraction job rows
  -> optional locally generated PDF demo asset
  -> normalized Markdown response
  -> persistent chunk rows plus card metadata
  -> TF-IDF retrieval
  -> persisted knowledge Agent conversations/messages/runs
```
...
When PostgreSQL + pgvector is enabled, the existing relational
`knowledge_documents` and `knowledge_chunks` tables should be extended with or
paired to `knowledge_embeddings` without changing the public API shape.
...
| `GET` | `/directories` | List demo directory items and tree |
| `POST` | `/directories` | Create a demo directory |
| `PUT` | `/directories/{directory_id}` | Update a demo directory |
| `POST` | `/directories/{directory_id}/move` | Move a demo directory |
...
| `POST` | `/agent/conversations` | Create or resume a user/page/document scoped knowledge Agent conversation |
| `GET` | `/agent/conversations` | List current user's knowledge Agent conversations |
| `GET` | `/agent/conversations/{conversation_id}/messages` | List messages for one conversation |
| `POST` | `/agent/conversations/{conversation_id}/messages` | Send a message, search knowledge evidence, create an Agent run, record tool call/memory, and write audit |

### Knowledge Agent Runtime

The knowledge-center chat is now backed by relational AI runtime tables:

```text
conversation
  -> user message
  -> knowledge.search tool call
  -> assistant message with evidence
  -> agent run steps
  -> memory entry
  -> audit log
```
...
- Conversation ownership is checked against the current user.
- The answer is generated by a local `knowledge-agent-v1` response helper, not a
  remote LLM call.
- Evidence comes from seeded/demo knowledge plus persisted document/chunk rows.
- The runtime tables make the chat observable and resumable. Document/chunk
  persistence is present, but embeddings and vector search are not production
  infrastructure yet.
...
- Account Center -> Data Assets Center
